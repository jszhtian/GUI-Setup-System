      ...
      for (i = 0; i < db.NumFiles; i++)
      {
        ...
        */
        ...
        if (!testCommand)
        {
		  ...
		  WCHAR outPath[1024] = { 0 };
		  const char* tmpstr = args[2];
		  int nLen = strlen(tmpstr)+1;
		  int newLen = MultiByteToWideChar(CP_ACP, 0, tmpstr, nLen, NULL, 0);
		  TCHAR tmpTchar[512];
		  MultiByteToWideChar(CP_ACP, 0, tmpstr, nLen, tmpTchar, newLen);
		  MyCreateDir(tmpTchar);
		  wcscpy(outPath, tmpTchar);
		  wcscat(outPath, L"/");
		  wcscat(outPath, name);
          const UInt16 *destPath = (const UInt16 *)outPath;
		  printf("\n");
		  printf("Path:");
